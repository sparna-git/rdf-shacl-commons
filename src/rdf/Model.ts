...
export class Model {
    ...
    readListContent(list: Term): Term[] {
        ...
        if (subLists.length > 0 && subLists[0]) {
    ...
    readRootList(listId: Term):Term | null {
        var root = this.readSuperList(listId);
        if (!root) {
            return listId as Term;
        } else {
            return this.readRootList(root);
        }
    }

    readSuperList(listId: Term):Term | undefined {
        const propertyQuads:Quad[] = this.store.getQuads(
            null,
            RDF.REST,
            listId,
            null
