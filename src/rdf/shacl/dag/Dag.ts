export class Dag<Payload> implements DagIfc<Payload> {
   ...
   */
  initFromIdableEntity(
    hierarchy: Map<string, string[]>,
    data: Array<Payload & { getId(): string }>,
  ): void {
   ...
   */
  initFromParentableAndIdAbleEntity(
    data: Array<Payload & { getId(): string } & { getParents(): string[] }>,
    disabled: string[],
  ): void {
    let dataMap: Map<string, Payload> = new Map<string, Payload>();
    data.forEach((item) => {
...
export class DagNode<Payload> implements DagNodeIfc<Payload> {
  ...
  toDebugString(depth: number): string {
    ...
    let result =
      indent + "id:" + this.id + (this.disabled ? " !disabled" : "") + "\n";
...
export interface DagNodeIfc<Payload> {
  ...
  traverseBreadthFirst: (
    processor: (node: DagNodeIfc<Payload>) => void,
  ) => void;
