...
export class NodeShape extends Shape {
  ...
  getShTargetClass(): Resource[] {
    return this.graph.readProperty(
      this.resource,
      SH.TARGET_CLASS,
    ) as Resource[];
  }
  ...
  getShProperty(): PropertyShape[] {
    return this.graph
      .readProperty(this.resource, SH.PROPERTY)
      .map(
        (node) => ShapeFactory.buildShape(node, this.graph) as PropertyShape,
      );
  }
  ...
  getLabel(lang: string): string | undefined {
    ...
    if (!label) {
      ...
      if (this.graph.hasTriple(this.resource, SH.TARGET_CLASS, null)) {
        let targetClass = this.graph.readSingleProperty(
          this.resource,
          SH.TARGET_CLASS,
        );
  ...
  getTooltip(lang: string): string | undefined {
    let tooltip = this.graph.readSinglePropertyInLang(
      this.resource,
      VOLIPI.MESSAGE,
      lang,
    )?.value;
    ...
    if (!tooltip) {
      // try rdfs:comment directly on the resource (for NodeShapes that are also classes)
      tooltip = this.graph.readSinglePropertyInLang(
        this.resource,
        RDFS.COMMENT,
        lang,
    ...
    if (!tooltip) {
      // try skos:definition directly on the resource
      tooltip = this.graph.readSinglePropertyInLang(
        this.resource,
        SKOS.DEFINITION,
        lang,
  ...
  getParents(): Resource[] {
    let parentsFromSuperClasses = this.#findShapesTargetingClassIn(
      this.getImmediateSuperClassesOfTargetClass(),
    );

    if (parentsFromSuperClasses.length == 0) {
      parentsFromSuperClasses = this.#findShapesTargetingClassIn(
        this.getSuperClassesOfTargetClass(2),
      );
    }
    if (parentsFromSuperClasses.length == 0) {
      parentsFromSuperClasses = this.#findShapesTargetingClassIn(
        this.getSuperClassesOfTargetClass(3),
      );
    }
    if (parentsFromSuperClasses.length == 0) {
      parentsFromSuperClasses = this.#findShapesTargetingClassIn(
        this.getSuperClassesOfTargetClass(4),
      );
    }
    if (parentsFromSuperClasses.length == 0) {
      parentsFromSuperClasses = this.#findShapesTargetingClassIn(
        this.getSuperClassesOfTargetClass(5),
      );
    }
    ...
    let parents = [
      ...new Set([...parentsFromSuperClasses, ...parentsFromShNode]),
    ];
    return parents;
  }
      ...
      classes
        ...
        .map((term) => {
          if (this.graph.hasTriple(term as Resource, RDF.TYPE, RDFS.CLASS)) {
            ...
            return this.graph.findSingleSubjectOf(
              SH.TARGET_CLASS,
              term,
            ) as Resource;
          }
  ...
  getChildren(): Resource[] {
    ...
    // concat parents from superclasses and from node - deduplicated
    let children = [
      ...new Set([...cildrenFromSuperClasses, ...childrenFromShNode]),
    ];
    return (
      children
  ...
  #getSuperClassesAtLevel(classNode: Term, level: number): Resource[] {
    ...
    if (level == 1) {
      ...
      var result: Resource[] = [];
      items.forEach((i) =>
        result.push(...this.#getSuperClassesAtLevel(i, level - 1)),
      );
      return result;
    }
  }
  ...
  #getInverseOfShNodeForExplicitNodeShapes(): Term[] {
    let subjects: Quad_Subject[] = this.graph.findSubjectsOf(
      SH.NODE,
      this.resource,
    );
  ...
  couldBeSkosConcept(): boolean {
    let isItselfSkosConcept: boolean =
      this.resource.value == SKOS.CONCEPT.value;
    let targetsSkosConcept: boolean =
      this.getShTargetClass().findIndex((t) => t.value == SKOS.CONCEPT.value) >
      -1;

    // read all sh:property, including sh:deactivated, ones with sh:hasValue, etc.
    let propShapes: Term[] = this.graph.readProperty(
      this.resource,
      SH.PROPERTY,
    );

    let hasPropertyRdfTypeSkosConcept: boolean =
      propShapes.findIndex((p) => {
        let propertyShape: PropertyShape = ShapeFactory.buildShape(
          p as Resource,
          this.graph,
        ) as PropertyShape;
        ...
        if (
          ...
          this.graph.hasTriple(p as Resource, SH.HAS_VALUE, SKOS.CONCEPT)
        ) {
          return true;
        }
    ...
    let hasConstraintOnSkosInScheme =
      propShapes.findIndex((p) => {
        let propertyShape: PropertyShape = ShapeFactory.buildShape(
          p as Resource,
          this.graph,
        ...
        if (
          ...
          this.graph.hasTriple(p as Resource, SH.HAS_VALUE, null)
        ) {
          return true;
        }
    ...
    return (
      isItselfSkosConcept ||
      targetsSkosConcept ||
      hasPropertyRdfTypeSkosConcept ||
      hasConstraintOnSkosInScheme
    );
  }
}
